                    ...
                    {categories.map(cat => {
                        ...
                        return (
                            <button key={cat} onClick={() => setActiveCategory(cat)}
                                className={`relative px-6 py-2 rounded-full text-sm font-semibold transition-colors ${active ? "text-stone-900" : "text-stone-500 hover:text-stone-900"} `} >
                                {active && (
                                    <motion.span
                ...
                <motion.div layout className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <AnimatePresence mode="popLayout">
                        {filtered.map(service => (
                            <motion.div key={service.name} layout initial={{ opacity: 0, y: 24 }}
                                animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 24 }}
                                transition={{ duration: 0.35, ease: "easeOut" }} whileHover={{ y: -4 }}
                                className="group relative bg-white rounded-2xl border border-stone-100 p-6 flex items-center justify-between cursor-pointer overflow-hidden" >
                                {/* Accent Line */}
                                ...
                                <div className="flex items-center gap-5">
                                    <div className="relative w-14 h-14 rounded-xl bg-stone-100 flex items-center justify-center">
                                        <img
                                            src={service.img}
                                            alt={service.name}
                                            className="w-9 h-9 transition-transform group-hover:scale-110"
                                        />
                                        <div className="absolute inset-0 rounded-xl bg-[#9fe870] blur-lg opacity-0 group-hover:opacity-30 transition-opacity" />
                                    </div>
